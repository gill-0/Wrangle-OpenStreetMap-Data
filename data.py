...
- node_id: the ref attribute starting at 0 of the nd tag i.e. what order the nd tag appears within
            the way element
 value of the nd tag
- position: the index
...
extract_last_re = re.compile('\S+$', re.IGNORECASE)
extract_prefix_re = re.compile('^\S+', re.IGNORECASE)
...
def get_node_way_att(element, attribs, FIELDS):
        for att in FIELDS:
            attribs[att] = element.attrib[att]
        #print "Node", '-' * 50
...
def get_tag_fields(element, child, tags, default_tag_type):
            ...
            # i think the key_type_split could be refactored better.
            key = child.attrib['k']
            ...
            if is_street_name(child):
                value = clean_street(value, street_mapping, extract_last_re)
                value = clean_street(value, cardinal_mapping, extract_prefix_re)
